	...
	public class MITrainMTSModelForm : System.Windows.Forms.Form
    {
        ...
        private void TrainModelFile()
        {
            ...
            for (int imodel = 0; imodel < nmodel; imodel++) {
                ...
                pinf.FileName = "\"" + str_train + "\"";//bug fix to handle folders with spaces in name: ZY
                //pinf.FileName = str_train ;
                pinf.Arguments = "\"" + Path.GetFullPath(MIConstDef.TrainingCfgFileName) + "\""; //bug fix to handle folders with spaces in name: ZY
